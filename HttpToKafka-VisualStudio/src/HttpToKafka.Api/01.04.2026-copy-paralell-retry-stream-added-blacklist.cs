...
public sealed class ParallelCopyOptions
{
    ...
    public string BlacklistFilePath { get; init; } = "BlackList.json";
}
...
public sealed class LocalJsonBlacklistStore : IBlacklistStore
{
    ...
    public async Task AppendAsync(BlacklistEntry entry, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(_filePath, output, ct).ConfigureAwait(false);
        }
        finally
        {
            _lock.Release();
...
public sealed class ParallelCopyRunner
{
    ...
    public async Task<ParallelCopyResult> RunAsync(CopyRequest req, ParallelCopyOptions opt, CancellationToken ct)
    {
        ...
        _log.LogInformation(
            "Parallel copy start. Bucket={Bucket}, Prefix={Prefix}, TargetContainer={Container}, DOP={Dop}, Capacity={Cap}, ResumeFromKey={ResumeFromKey}, InitialModifiedAfter={After}, ProgressLogInterval={ProgressLogInterval}, MaxReadRetries={MaxReadRetries}, BlacklistFilePath={BlacklistFilePath}",
            req.SourceBucket, req.SourcePrefix, req.TargetContainer, opt.MaxDegreeOfParallelism, opt.BoundedCapacity,
            resumeFromKey, effectiveAfter, opt.ProgressLogInterval, opt.MaxReadRetries, opt.BlacklistFilePath);
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        var blacklistStore = new LocalJsonBlacklistStore("BlackList.json");
