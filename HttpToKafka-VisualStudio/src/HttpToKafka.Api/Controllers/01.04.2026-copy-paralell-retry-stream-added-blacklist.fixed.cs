...
{
    Task AppendAsync(BlacklistEntry entry, CancellationToken ct);
}
...
public sealed class ParallelCopyOptions
{
    ...
    public string BlacklistBlobName { get; init; } = "BlackList.json";
}
...
public sealed class EcsS3ObjectSource : IEcsObjectSource, IDisposable
{
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct)
    {
        ...
        do
        {
            ...
            {
                var lastModified = o.LastModified ?? DateTime.MinValue;
                var lm = new DateTimeOffset(DateTime.SpecifyKind(lastModified, DateTimeKind.Utc));
                var size = o.Size ?? 0L;
                ...
                yield return new EcsObjectDescriptor(bucket, o.Key, size, lm, o.ETag);
            }
...
public sealed class BlobJsonBlacklistStore : IBlacklistStore
{
    private readonly BlobContainerClient _container;
    ...
    public BlobJsonBlacklistStore(BlobCheckpointStoreOptions opt, string blobName)
    {
        var svc = new BlobServiceClient(opt.BlobServiceUri, new DefaultAzureCredential());
        _container = svc.GetBlobContainerClient(opt.Container);
        _blobName = blobName;
    ...
    public async Task AppendAsync(BlacklistEntry entry, CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                using var reader = new StreamReader(dl.Value.Content, Encoding.UTF8);
                var json = await reader.ReadToEndAsync().ConfigureAwait(false);
                entries = string.IsNullOrWhiteSpace(json)
                    ? new List<BlacklistEntry>()
                    : JsonSerializer.Deserialize<List<BlacklistEntry>>(json, JsonOpt) ?? new List<BlacklistEntry>();
            }
            catch (Azure.RequestFailedException ex) when (ex.Status == 404)
            {
                entries = new List<BlacklistEntry>();
            }

            entries.Add(entry);

            var output = JsonSerializer.Serialize(entries, JsonOpt);
...
public sealed class ParallelCopyRunner
{
    ...
    private long _blacklistedCount;
    ...
    public async Task<ParallelCopyResult> RunAsync(CopyRequest req, ParallelCopyOptions opt, CancellationToken ct)
    {
        ...
        Interlocked.Exchange(ref _blacklistedCount, 0);
        ...
        _log.LogInformation(
            "Parallel copy start. Bucket={Bucket}, Prefix={Prefix}, TargetContainer={Container}, DOP={Dop}, Capacity={Cap}, ResumeFromKey={ResumeFromKey}, InitialModifiedAfter={After}, ProgressLogInterval={ProgressLogInterval}, MaxReadRetries={MaxReadRetries}, BlacklistBlobName={BlacklistBlobName}",
            req.SourceBucket, req.SourcePrefix, req.TargetContainer, opt.MaxDegreeOfParallelism, opt.BoundedCapacity,
            resumeFromKey, effectiveAfter, opt.ProgressLogInterval, opt.MaxReadRetries, opt.BlacklistBlobName);
            ...
            .Select(i => Task.Run(async () =>
            {
                try
                {
                    await foreach (var item in channel.Reader.ReadAllAsync(token).ConfigureAwait(false))
                    {
                        ...
                        try
                        {
                            await CopyObjectWithRetryAsync(req, opt, obj, blobName, metadata, token).ConfigureAwait(false);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Interlocked.Increment(ref _processedCount);
                            Interlocked.Increment(ref _blacklistedCount);

                            await _blacklistStore.AppendAsync(
                                new BlacklistEntry(
                                    obj.Bucket,
                                    obj.Key,
                                    obj.Size,
                            ...
                            _log.LogError(ex, "Copy failed for {Key} (seq={Seq}) after retries. Object added to blacklist and migration will continue.", obj.Key, item.Seq);
        ...
        var blacklisted = (int)Interlocked.Read(ref _blacklistedCount);

        _log.LogInformation(
            "Parallel copy finished. Copied={Copied}, Failed={Failed}, Blacklisted={Blacklisted}, Processed={Processed}, Watermark=({WmLm},{WmKey})",
            copied, failed, blacklisted, processed, finalWm.LastModifiedUtc, finalWm.Key);

        return new ParallelCopyResult(copied, failed, processed, finalWm.Key, finalWm.LastModifiedUtc);
    }

    private async Task CopyObjectWithRetryAsync(
        CopyRequest req,
        ...
        CancellationToken ct)
    {
        ...
        var attempts = Math.Max(1, opt.MaxReadRetries);
        ...
        for (var attempt = 1; attempt <= attempts; attempt++)
        {
            ...
            try
            {
                await using var stream = await _source.OpenReadAsync(obj.Bucket, obj.Key, ct).ConfigureAwait(false);
                await _sink.UploadAsync(req.TargetContainer, blobName, stream, metadata, ct).ConfigureAwait(false);
                return;
            }
            catch (Exception ex) when (attempt < attempts && IsRetryableReadException(ex))
            {
                lastException = ex;
                var delay = TimeSpan.FromMilliseconds(opt.RetryBaseDelay.TotalMilliseconds * attempt);
                _log.LogWarning(ex, "Retryable read/copy failure for {Key}. Attempt {Attempt}/{MaxAttempts}. Retrying after {Delay}.",
                    obj.Key, attempt, attempts, delay);
        ...
        throw lastException ?? new InvalidOperationException($"Copy failed for object '{obj.Key}'.");
    }
    ...
    private static bool IsRetryableReadException(Exception ex)
    {
        ...
        {
            var status = (int)s3ex.StatusCode;
        ...
        {
            if (azex.Status == 408 || azex.Status == 429 || azex.Status >= 500)
                return true;
        }

        return ex.InnerException is not null && IsRetryableReadException(ex.InnerException);
    }
        ...
        int workerId)
    {
        ...
        var blacklisted = Interlocked.Read(ref _blacklistedCount);

        _log.LogInformation(
            "[Progress] Index={Index}, Copied={Copied}, Failed={Failed}, Blacklisted={Blacklisted}, Processed={Processed}, ObjectsPerSecond={ObjectsPerSecond:F2}, MBPerSecond={MBPerSecond:F2}, CurrentKey={Key}, TargetBlob={Blob}, Worker={Worker}",
            currentIndex, copied, failed, blacklisted, processed, objectsPerSecond, mbPerSecond, obj.Key, blobName, workerId);
    }
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        var blacklistStore = new BlobJsonBlacklistStore(
            new BlobCheckpointStoreOptions
            {
                BlobServiceUri = azureBlobServiceUri,
                Container = "ecs-copy-state"
            },
        ...
        var options = new ParallelCopyOptions
        {
            ...
            BlacklistBlobName = "BlackList.json"
        };
