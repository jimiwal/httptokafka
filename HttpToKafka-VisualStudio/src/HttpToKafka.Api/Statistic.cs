...
// Program:
// 1) znajduje 10 000 najmniejszych obiektów w bucket
...
internal static class Program
{
    ...
    private const string ECS_REGION = "us-east-1";   // dla AWS SDK wymagane (dla ECS dowolne sensowne)
    private const bool ECS_FORCE_PATH_STYLE = true;      // ECS zwykle wymaga

    private const string BUCKET_NAME = "your-bucket-name";

    // ile najmniejszych obiektów wybieramy
    private const int TARGET_COUNT = 10_000;
    ...
    private static async Task Main()
    {
        ...
        Console.WriteLine($"Select : {TARGET_COUNT:N0} smallest");
        Console.WriteLine();

        // 1) Find smallest
        Console.WriteLine("Listing objects and selecting smallest...");
        ...
        var stats = await DownloadAndMeasureAsync(s3, BUCKET_NAME, smallest, CancellationToken.None);
    ...
    {
        // PriorityQueue is min-heap; we want to keep k smallest -> implement as max-heap by using priority = -Size.
        ...
        CancellationToken ct)
    {
        ...
        for (int i = 0; i < objs.Count; i++)
        {
            ...
            await using (resp.ResponseStream.ConfigureAwait(false))
            {
                ...
                _ = ms.Length; // touch to avoid weird optimizations
            }
            ...
            if ((i + 1) % 500 == 0)
            {
                ...
                var ops = (i + 1) / Math.Max(0.000001, elapsed);
                Console.WriteLine($"  downloaded: {i + 1:N0}/{objs.Count:N0} | avg objs/sec so far: {ops:F2}");
            }
        }
        ...
        // meaningful when we have at least 1 full minute bucket; but we still compute if >=1
