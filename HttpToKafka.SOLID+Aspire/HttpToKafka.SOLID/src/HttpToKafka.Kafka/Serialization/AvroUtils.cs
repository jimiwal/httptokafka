...
internal static class AvroUtils
{
    ...
    public static Dictionary<string, object?> GenericRecordToDictionary(GenericRecord rec)
    {
        var dict = new Dictionary<string, object?>();
        foreach (var f in rec.Schema.Fields)
        {
            dict[f.Name] = rec.TryGetValue(f.Name, out var v) ? v : null;
        }
        return dict;
    }
